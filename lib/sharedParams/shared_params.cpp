/************************************************************************************//**
* \file         Demo/ARMCM4_STM32F4_Nucleo_F429ZI_GCC/Boot/shared_params.c
...
#include <stdio.h>
#include "shared_params.hpp"                             /* Shared parameters header.    */
 ...
 */
#define SHARED_PARAMS_BUFFER_MAGIC                 (0xCE42E7A2u)
...
typedef struct __attribute__((__packed__)) t_shared_params_buffer
{
...
void SharedParamsInit(void)
{
  ...
  if (!SharedParamsValidateBuffer())
  {
     ...
     */
    sharedParamsBuffer.identifier = SHARED_PARAMS_BUFFER_MAGIC;
...
static bool SharedParamsValidateBuffer(void)
{
  ...
  /* Perform validation. */
  if ( (sharedParamsBuffer.identifier == SHARED_PARAMS_BUFFER_MAGIC) &&
