...
# ^original headers before modification/rewrite for Renstall
# Hosted on https://github.com/jedrzejboczar/elf-size-analyze
# by Jędrzej Boczar 
...
ERROR: No symbols from given section found or all were ignored!
    ...
    if args.ram:
        filtered_symbols = filter_symbols(lambda sec: sec and sec.occupies_ram())
        print_tree('RAM', filtered_symbols)
...
def get_sections(filename, verbose=False):
    """ return all segments of the given elf file
    """

    oride_args = [sys.argv[0]]
    oride_args.append(filename)
    oride_args.append('-P')
